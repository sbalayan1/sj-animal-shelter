class Application
    ...
    def call(env)
      ...
      if req.path.match(/animals/) && req.get?
        ...
        return [200, { 'Content-Type' => 'application/json' }, [ {:visitor => res_visitor}.to_json ]]
      ...
      elsif req.path.match(/adoptions/) && req.post?

        data = JSON.parse req.body.read
        adoption = Adoption.create(date: data["date"], price: data["price"], animal_id: data["animal_id"], visitor_id: data["visitor_id"])

        res_adoption = {id: adoption.id, date: adoption.date, price: adoption.price, animal_id: adoption.animal_id, visitor_id: adoption.visitor_id}
        
        return [200, { 'Content-Type' => 'application/json' }, [ {:adoption => res_adoption}.to_json ]]
